...
beforeEach(() => {
  ...
  vi.mocked(mintRetrievalToken).mockReset().mockResolvedValue({
    token: 'rtok_mock',
...
describe('orchestratePostAuth — failure branches', () => {
  ...
  it('marks FAILED and re-throws when mintRetrievalToken throws', async () => {
    findTxn()
      .mockResolvedValue(buildTxn());
    vi.mocked(mintRetrievalToken).mockRejectedValue(new Error('vault_entry_not_found'));
    vi.mocked(getPaymentProvider).mockReturnValue(buildProvider());
