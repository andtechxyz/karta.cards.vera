...
export async function storeCard(input: StoreInput): Promise<StoreResult> {
  ...
  // Dedup check.  Fingerprint is unique at the schema level; we check first
  // so we can emit a clean DUPLICATE_REJECTED event either way.
  ...
  if (existing) {
    ...
    // reuse — return the existing entry without emitting CREATE.
    return {
      vaultEntryId: existing.id,
      panLast4: existing.panLast4,
      deduped: true,
  ...
  // Encrypt the sensitive payload.  We keep the structure JSON so CVC is
  // optional without a second column.
