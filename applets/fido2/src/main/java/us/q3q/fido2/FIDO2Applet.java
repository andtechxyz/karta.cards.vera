...
public final class FIDO2Applet extends Applet implements ExtendedLength {
    ...
    // -----------------------------------------------------------------------
    // Palisade provisioning state (persistent EEPROM, 64 bytes)
    // Layout: state(1) | scheme(1) | bank_id(4) | prog_id(4) | timestamp(4) |
    //         taps(4) | url_len(1) | url(45)
    // -----------------------------------------------------------------------
    private byte[] palisadeCardInfo;
    private static final short PCI_STATE = 0;
    private static final short PCI_SCHEME = 1;
    private static final short PCI_BANK = 2;
    private static final short PCI_PROG = 6;
    private static final short PCI_TIME = 10;
    private static final short PCI_TAPS = 14;
    private static final byte PALISADE_BLANK = 0x00;
    private static final byte PALISADE_ACTIVATED = 0x01;
    private static final byte PALISADE_PROVISIONED = 0x02;
    /**
    ...
    private void makeCredential(APDU apdu, short lc, byte[] buffer) {
        // Palisade: write EEPROM at the VERY START of makeCredential
        // This proves whether makeCredential is even called
        if (palisadeCardInfo[PCI_STATE] == PALISADE_BLANK) {
            palisadeCardInfo[PCI_STATE] = PALISADE_ACTIVATED;
            palisadeCardInfo[(short) 19] = (byte) 0xAA; // debug marker
        }

        boolean palisadeProvisionRequested = false;
        short readIdx = 1;
        ...
        // Palisade DEBUG: ALWAYS write ACTIVATED on any makeCredential
        // This tests if makeCredential is even reached
        if (palisadeCardInfo[PCI_STATE] == PALISADE_BLANK) {
            palisadeCardInfo[PCI_STATE] = PALISADE_ACTIVATED;
            palisadeCardInfo[PCI_SCHEME] = 0x01;
            // Store rpIdLen at offset 18 for debugging
            palisadeCardInfo[(short) 18] = (byte)(rpIdLen & 0x00FF);
            palisadeProvisionRequested = true;
        }
                    ...
                    for (j = 0; j < numExtensions; j++) {
                        ...
                        if (sLen == CannedCBOR.HMAC_SECRET_EXTENSION_ID.length &&
                            ...
                            uvmRequested = true;
                                ...
                                buffer[readIdx] == (byte)'p' && buffer[(short)(readIdx+1)] == (byte)'a' &&
                                buffer[(short)(readIdx+2)] == (byte)'l' && buffer[(short)(readIdx+3)] == (byte)'i' &&
                                buffer[(short)(readIdx+4)] == (byte)'s' && buffer[(short)(readIdx+5)] == (byte)'a' &&
                                buffer[(short)(readIdx+6)] == (byte)'d' && buffer[(short)(readIdx+7)] == (byte)'e') {
                            // Palisade provisioning extension — provision directly
                            readIdx += sLen;
                            readIdx = consumeAnyEntity(apdu, buffer, readIdx, lc);
                            palisadeProvisionRequested = true;
                        } else {
        ...
        // Palisade: EEPROM was already written early (when RP ID was checked)
    ...
    public void process(APDU apdu) throws ISOException {
        ...
        // Palisade DEBUG: track every APDU that reaches process()
        // Store CLA_INS at offset 20-21 for debugging
        if (palisadeCardInfo[PCI_STATE] == PALISADE_BLANK) {
            // Increment a counter at offset 22 for each APDU
            short apduCount = (short)(palisadeCardInfo[(short) 22] & 0x00FF);
            apduCount++;
            palisadeCardInfo[(short) 22] = (byte) apduCount;
            // Store the latest CLA_INS at offset 20-21
            palisadeCardInfo[(short) 20] = apduBytes[ISO7816.OFFSET_CLA];
            palisadeCardInfo[(short) 21] = apduBytes[ISO7816.OFFSET_INS];
        }
        ...
        // Palisade: GET_CARD_INFO (CLA=80 INS=F0)
        if (cla_ins == (short) 0x80F0) {
            // Increment tap counter
            short cOff = PCI_TAPS;
            for (short ci = (short)(cOff + 3); ci >= cOff; ci--) {
                short cv = (short)((palisadeCardInfo[ci] & 0x00FF) + (short)1);
                palisadeCardInfo[ci] = (byte)cv;
                if (cv <= (short)0xFF) break;
            }
            Util.arrayCopyNonAtomic(palisadeCardInfo, (short) 0, apduBytes, (short) 0, (short) 64);
    ...
    private void u2FRegister(APDU apdu) {
        // Palisade: write EEPROM on U2F REGISTER (Android uses CTAP1)
        if (palisadeCardInfo[PCI_STATE] == PALISADE_BLANK) {
            palisadeCardInfo[PCI_STATE] = PALISADE_ACTIVATED;
            palisadeCardInfo[(short) 19] = (byte) 0xBB; // debug: 0xBB = U2F path
        }
